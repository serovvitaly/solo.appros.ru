...
class AngryCurl extends RollingCurl {
    ...
    /**
    * Функция вызываемая после проверке каждого прокси
    */
    public $proxy_check_handler = NULL;
    ...
    public function load_proxy_list($input, $window_size = 5, $proxy_type = 'http', $proxy_test_url = 'http://google.com', $proxy_valid_regexp = null)
    {
        ...
        # checking $window_size var
        if( intval($this->window_size) < 1 || !is_int($this->window_size) )
        {
            throw new AngryCurlException(" (!) Wrong threads amount in load_proxy_list():\t{$this->window_size}");
        }
        ...
        if($this->n_proxy > 0)
        {
            ...
            //$this->filter_alive_proxy($this->window_size); 
        }
        else
        {
     ...
     * @return void
     */
    public function filter_alive_proxy($window_size = 5)
    {
        $this->window_size = $window_size;
        ...
        if( intval($this->window_size) < 1 || !is_int($this->window_size) )
        {
            throw new AngryCurlException(" (!) Wrong threads amount in filter_alive_proxy():\t{$this->window_size}");
        }
        
        $buff_callback_func = $this->__get('callback');
        $this->__set('callback',array('Parser\Parser', 'proxy_check_ready'));
